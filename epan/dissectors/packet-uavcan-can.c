...
dissect_uavcan(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void *data)
{
    ...
    /* Semi-unique id for reassembly lookup table note transfer-ID rolls-over every 32 times  */

    DISSECTOR_ASSERT(data);
    can_info = *((struct can_info *) data);

    tail_byte = tvb_get_guint8(tvb, tvb_reported_length(tvb) - 1);
    ...
    ti = proto_tree_add_item(tree, proto_uavcan, tvb, offset, tvb_reported_length(tvb), ENC_NA);
    uavcan_tree = proto_item_add_subtree(ti, ett_uavcan);

    can_id_tree = proto_tree_add_subtree_format(uavcan_tree, tvb, 0, 0,
                                                ett_uavcan_can, NULL, "CAN ID field: 0x%08x",
                                                can_info.id);
    proto_item_set_generated(can_id_tree);
    ...
    if (UAVCAN_IS_MESSAGE(can_info.id)) {
        ...
        /* Set source address */
        src_addr = (guint16 *) wmem_alloc(pinfo->pool, 2);
        *src_addr = (guint16) UAVCAN_SOURCE_ID(can_info.id);
        ...
        dest_addr = (guint16 *) wmem_alloc(pinfo->pool, 2);
    ...
    can_data_tree = proto_tree_add_subtree(uavcan_tree, tvb, 0, tvb_reported_length(
                                               tvb), ett_uavcan_message, NULL, "CAN data field");

    proto_tree_add_item(can_data_tree, hf_uavcan_start_of_transfer, tvb,
                        tvb_captured_length(tvb) - 1, 1, ENC_NA);
    ...
    proto_tree_add_item(can_data_tree, hf_uavcan_transfer_id, tvb,
                        tvb_captured_length(tvb) - 1, 1, ENC_NA);
    proto_tree_add_item(can_data_tree, hf_uavcan_data, tvb, 0, tvb_captured_length(tvb) - 1,
                        ENC_NA);
    ...
    if ((tail_byte & (START_OF_TRANSFER | END_OF_TRANSFER)) ==
        ...
        tvb_set_reported_length(tvb, tvb_reported_length(tvb) - 1); /* Don't pass Tail byte to DSDL */
    ...
    if ((tail_byte & (START_OF_TRANSFER | END_OF_TRANSFER)) !=
        ...
        if (!pinfo->fd->visited) {  /* Not visited */
            ...
            if (reassembled) {
                ...
                multi_tree = proto_tree_add_subtree(uavcan_tree, reassembled_tvb, 0,
                                                    tvb_reported_length(
                                                        reassembled_tvb), ett_uavcan_message, NULL,
                                                    "Multi-frame");
