...
dissect_list_service_data(tvbuff_t *tvb, int tvb_offset, proto_tree *tree, gboolean is_request)
{
    ...
    if (is_request == TRUE) {
        ...
        guint8 str_len = tvb_get_guint8(tvb, tvb_offset);
        ti = proto_tree_add_item(tree, hf_register_name,
                                 tvb, tvb_offset + 1, str_len, ENC_ASCII|ENC_NA);
...
dissect_access_service_data(tvbuff_t *tvb, int tvb_offset, proto_tree *tree, gboolean is_request)
{
    ...
    if (is_request == TRUE) {
        ...
        ti = proto_tree_add_item(tree, hf_uavcan_time_syncronizedtimestamp,
                                 tvb, offset, 7, ENC_LITTLE_ENDIAN);
        proto_item_set_generated(ti);
        offset += 7;
        ti = proto_tree_add_item(tree, hf_register_access_mutable,
                                 tvb, offset, 1, ENC_NA);
        proto_item_set_generated(ti);
        ti = proto_tree_add_item(tree, hf_register_access_persistent,
                                 tvb, offset, 1, ENC_NA);
        proto_item_set_generated(ti);
    ...
    ti = proto_tree_add_item(tree, hf_register_value_tag,
                             tvb, offset, 1, ENC_NA);
    proto_item_set_generated(ti);
    offset += 1;

    if (tag == 1) { /* String */
        ti = proto_tree_add_item(tree, hf_register_value_size,
                                 tvb, offset, 1, ENC_NA);
        proto_item_set_generated(ti);
        str_len = tvb_get_guint8(tvb, offset);
        offset += 1;
        ...
        /*ti = proto_tree_add_item(tree, hf_uavcan_data, tvb, 0, tvb_captured_length(tvb), ENC_NA);
         * proto_item_set_generated(ti);*/
    } else {
        guint8 array_len = tvb_get_guint8(tvb, offset);

        if (array_len == 0 || tag == 0) {
            ti = proto_tree_add_item(tree, hf_uavcan_primitive_Empty,
                                     tvb, 0, 0, ENC_NA);
        } else {
            ti = proto_tree_add_item(tree, hf_register_value_size,
                                     tvb, offset, 1, ENC_NA);
            ...
            for (guint8 i = 0; i < array_len; i++) {
                switch (tag) {
                case 4:     /*Integer64*/
                    ti = proto_tree_add_item(tree, hf_uavcan_primitive_array_Integer64,
                                             tvb, offset, 8, ENC_LITTLE_ENDIAN);
                    offset += 8;
                    break;

                case 5:     /*Integer32*/
                    ti = proto_tree_add_item(tree, hf_uavcan_primitive_array_Integer32,
                                             tvb, offset, 4, ENC_LITTLE_ENDIAN);
                    offset += 4;
                    break;

                case 6:     /*Integer16*/
                    ti = proto_tree_add_item(tree, hf_uavcan_primitive_array_Integer16,
                                             tvb, offset, 2, ENC_LITTLE_ENDIAN);
                    offset += 2;
                    break;

                case 7:     /*Integer8*/
                    ti = proto_tree_add_item(tree, hf_uavcan_primitive_array_Integer8,
                                             tvb, offset, 1, ENC_LITTLE_ENDIAN);
                    offset += 1;
                    break;

                case 8:     /*Natural64*/
                    ti = proto_tree_add_item(tree, hf_uavcan_primitive_array_Natural64,
                                             tvb, offset, 2, ENC_LITTLE_ENDIAN);
                    offset += 8;
                    break;

                case 9:     /*Natural32*/
                    ti = proto_tree_add_item(tree, hf_uavcan_primitive_array_Natural32,
                                             tvb, offset, 2, ENC_LITTLE_ENDIAN);
                    offset += 4;
                    break;

                case 10:     /*Natural16*/
                    ti = proto_tree_add_item(tree, hf_uavcan_primitive_array_Natural16,
                                             tvb, offset, 2, ENC_LITTLE_ENDIAN);
                    offset += 2;
                    break;

                case 11:     /*Natural8*/
                    ti = proto_tree_add_item(tree, hf_uavcan_primitive_array_Natural8,
                                             tvb, offset, 2, ENC_LITTLE_ENDIAN);
                    offset += 1;
                    break;

                case 12:     /*Real64*/
                    ti = proto_tree_add_item(tree, hf_uavcan_primitive_array_Real64,
                                             tvb, offset, 8, ENC_LITTLE_ENDIAN);
                    offset += 8;
                    break;

                case 13:     /*Real16*/
                    ti = proto_tree_add_item(tree, hf_uavcan_primitive_array_Real32,
                                             tvb, offset, 8, ENC_LITTLE_ENDIAN);
                    offset += 8;
                    break;

                case 14:     /*Real8*/
                    ti = proto_tree_add_item(tree, hf_uavcan_primitive_array_Real16,
                                             tvb, offset, 8, ENC_LITTLE_ENDIAN);
                    offset += 8;
                    break;
...
dissect_dsdl_message(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void *data)
{
    ...
    if (id == 7509) {
        ...
        proto_tree_add_item(tree, hf_uavcan_primitive_String, tvb, 9, tvb_get_guint8(tvb, 8),
                            ENC_NA|ENC_ASCII);
        return tvb_captured_length(tvb);
    }
...
dissect_dsdl_service_request(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void *data)
{
    ...
    if (id == 384) { /* Dissect Access.1.0 frame */
        ...
        proto_tree_add_item(tree, hf_uavcan_getinfo_path,
                            tvb, 1, tvb_get_guint8(tvb, 0), ENC_NA|ENC_ASCII);
        return tvb_captured_length(tvb);
        ...
        proto_tree_add_item(tree, hf_uavcan_directory_path,
                            tvb, 9, tvb_get_guint8(tvb, 8), ENC_NA|ENC_ASCII);
        return tvb_captured_length(tvb);
        ...
        proto_tree_add_item(tree, hf_uavcan_modify_overwrite_destination,
                            tvb, 0, 1, ENC_NA);
        guint8 source_len = tvb_get_guint8(tvb, 4);
        proto_tree_add_item(tree, hf_uavcan_modify_source_path,
                            tvb, 5, source_len, ENC_NA|ENC_ASCII);
        proto_tree_add_item(tree, hf_uavcan_modify_destination_path,
                            tvb, 6 + source_len, tvb_get_guint8(tvb, 5 + source_len), ENC_NA|ENC_ASCII);
        return tvb_captured_length(tvb);
        ...
        proto_tree_add_item(tree, hf_uavcan_read_path,
                            tvb, 6, tvb_get_guint8(tvb, 5), ENC_NA|ENC_ASCII);
        return tvb_captured_length(tvb);
    } else if (id == 409) { /* Dissect Write.1.X frame */
        proto_tree_add_item(tree, hf_uavcan_write_offset,
                            tvb, 0, 5, ENC_LITTLE_ENDIAN);
        guint8 path_len = tvb_get_guint8(tvb, 5);
        proto_tree_add_item(tree, hf_uavcan_write_path,
                            tvb, 6, path_len, ENC_NA|ENC_ASCII);
        guint16 data_len = tvb_get_guint16(tvb, 6 + path_len, ENC_LITTLE_ENDIAN);
        proto_tree_add_item(tree, hf_uavcan_primitive_Unstructured,
                            tvb, 8 + path_len, data_len, ENC_NA|ENC_ASCII);
        return tvb_captured_length(tvb);
    }
...
dissect_dsdl_service_response(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void *data)
{
    ...
    if (id == 384) { /* Dissect Access.1.0 frame */
        ...
        proto_tree_add_item(tree, hf_uavcan_entry_base_name,
                            tvb, 5, tvb_get_guint8(tvb, 4), ENC_NA|ENC_ASCII);
        return tvb_captured_length(tvb);
    } else if (id == 407) { /* Dissect Modify.1.X frame */
